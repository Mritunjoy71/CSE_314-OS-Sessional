...
input="/media/mritunjoy/photos/assignment/allfiles/absents.txt"
while IFS='' read -r line || [[ -n "$line" ]]; do
    echo "$line -0" >> /media/mritunjoy/photos/assignment/allfiles/marks.txt
done < "$input"

sort -n -k 4 absents.txt > absents_roll.txt
sort -n -k 4 marks.txt > sub_marks.txt

cp absents_roll.txt /media/mritunjoy/photos/assignment/Output
cp sub_marks.txt /media/mritunjoy/photos/assignment/Output
